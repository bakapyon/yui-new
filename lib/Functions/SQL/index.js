...
const databaseSQL = path.normalize(`${yuiPath}/lib/Databases/Informations/${envInfo?.parameters?.databaseName?.value || 'general.db'}`);
const dialogsSQL = path.normalize(`${yuiPath}/lib/Databases/Informations/dialogues.db`);
const extrasSQL = path.normalize(`${yuiPath}/lib/Databases/Utilities/extras.db`);
...
function fixDatabase(params = {}) {
    ...
    try {
        ...
        const oldDatabase = path.normalize(`${yuiPath}/lib/Databases/Informations/${settings.oldDb}`);
...
function dialoguePicker(
    ...
    try {
        ...
        if (nameKey instanceof Error && taskName === 'S.E.R') {
            ...
            /* Local */
            replaceData.path = path.dirname(path.resolve(stderr[0] || './')).replace(yuiPath, '').replace('\\', '');
...
/* Adquire uma palavra ou dado de uma determinada table */
function getKeyword(
    selectLang = region,
    ...
    try {
        ...
        /* Cónstroi a query SQLite3 */
        const query = getLists.command || `SELECT json_object('id', id, 'type', type, 'language', language, 'word', name) FROM ${getLists.table} ${getLists.language === 'any' ? '' : `WHERE language = '${getLists.language}'`} ${getLists.random ? 'ORDER BY RANDOM()' : `AND type = '${getLists.name}'`} LIMIT 1;`;

        /* Obtém a palavra */
        const wordSelect = Indexer('shell').bash(`sqlite3 "${extrasSQL}"`, query);
        envInfo.results.value = wordSelect.value.trim();
...
function resetAmbient(
    ...
    try {
        ...
        /* Insere a getKeyword na envInfo */
        envInfo.functions.keywords.value = getKeyword;
        ...
        module.exports = {
            [envInfo.name]: {
                ...
                [envInfo.exports.keywords]: envInfo.functions.keywords.value,
