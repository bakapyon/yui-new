...
function refStarter(
    ...
    return new Promise((resolve) => {
        ...
        try {
            ...
            app.use(express.static(`${yuiPath}/lib/Wiki/Extras`));
            ...
            wss.on('connection', (ws) => {
                ...
                /* Diz que conectou */
                ws.send(JSON.stringify({ success: true, startlog: true, message: 'Conexão estabelecida com sucesso!' }));
            });
            ...
            app.get('/homepage', (req, res) => {
                ...
                app.use(express.static(`${yuiPath}/lib/Wiki/Homepage`));
                ...
                try {
                    ...
                    if (jsonData?.password === config.secretKey.value && jsonData?.username === config.yourName.value && Object.keys(jsonData).length > 0) {
                        ...
                        if (isValidMessage(jsonData)) {
                            ...
                            /* Tenta fazer parse, se for um envio bash */
                            try {
                                useData.message = JSON.parse(jsonData.message)
                                useData.quoted = JSON.parse(jsonData.quoted)
                                useData.code = JSON.parse(jsonData.code)
                                useData.chatId = JSON.parse(jsonData.chatId)

                                /* Se der erros, tenta com padrão mesmo */
                            } catch (err) { /* Faz nada, pois já está padrão */}
            ...
            /* Cria a conexão socket interna da Yui */
