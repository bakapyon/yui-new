            ...
            if (isGroup && !isGroupAdmins && !isGroupCreator && !isOwner && !isModerator && isBotGroupAdmins) {
                ...
                if (conditions === false && functions.antilinks.type === 3 && functions.antilinks.enable === true) {
                    ...
                    for (let i = 0; i < urlData.hostnames.length; i += 1) {
                        ...
                        const isBadURL = Indexer('bash').bash(`bash "${path.normalize(`${yuiPath}/lib/Scripts/Hosts.sh`)}" "${urlData.hostnames[i]}" "${path.normalize(`${yuiPath}/lib/Databases/Utilities/hosts.txt`)}"`).value;
                ...
                if (conditions) {
                    /* Se ainda estiver no grupo, como casos de spam ou adm remover antes da Yui */
