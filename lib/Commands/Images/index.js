    ...
    try {
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            if (arks.includes('--help-dev') && isOwner === true) {
                ...
                /* Define a imagem */
                const imageLocated = await Indexer('images').search({
                    ...env.value,
                });

                /* Define se vai mandar como sticker */
                if (argl.includes('-sticker') || command === 'getsticker') {
                    /* Constrói o sticker */
                    const sticker = new Sticker(imageLocated.value, {
                        ...stickerConfig,
                        type: 'default',
                    ...
                    /* Se qualquer outro tipo */
                } else {
                    /* Envia como imagem */
                    envInfo.results.value = await kill.sendMessage(
                        chatId,
                        { image: { url: imageLocated.value }, caption: imageLocated.details },
                        reply,
                    );
                }
            }
        ...
        */
        ...
        /* Envia uma imagem random com os detalhes de erro, pra não quebrar a ideia do comando */
        await kill.sendMessage(env.value.chatId, {
            image: { url: path.normalize(`${__dirname}/Cache/Error.png`) },
            caption: Indexer('sql').languages(region, 'S.E.R', error, true, true, {
                command: 'IMAGES',
