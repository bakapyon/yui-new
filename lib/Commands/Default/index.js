...
let externalCodes = JSON.parse(fs.readFileSync(`${yuiPath}/lib/Databases/Configurations/external.json`));
            ...
            switch (commandFound) {
                ...
                */
                case caseChecker('menu', true, isCmd):
                    const menuText = Indexer('bash').bash(`bash "${yuiPath}/lib/Scripts/Menu.sh" "${argl[0]}" ${JSON.stringify(Indexer('sql').languages(region, 'Helper', 'Menu', true, true, env.value).value)} "${prefix}"`).value;
