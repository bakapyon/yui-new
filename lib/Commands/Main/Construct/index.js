...
let arrayOfCommands = JSON.parse(Indexer('bash').bash(`bash "${yuiPath}/lib/Scripts/Menu.sh" "array"`).value);
...
async function dataCollect(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof message === 'object') {
            ...
            /* Define se deve ignorar o processamento das mensagens da Yui, editadas, broadcast e reações como mensagens normais */
            if (conditions) {
                /* Define soft error */
            ...
            try {
                ...
                console.log('\x1b[31m', 'REPORT THIS TO PROJECT Yui [https://github.com/KillovSky/Yui]!\n', `\x1b[33m${JSON.stringify(message)}`);
            ...
            messageData.isBotGroupAdmins = messageData.groupAdmins?.includes(global.yuiNumber) || false;
            ...
            /* Define se é a Yui */
            messageData.isBot = chatMessage?.fromMe || global.yuiNumber === messageData.user || false;
            ...
            const stckPack = (removeAccents(config.stickerPack.value) || '🔰 Horizon [linktr.ee/killovsky] Yui ⚜️');
