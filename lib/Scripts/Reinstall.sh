...
if [[ "$latestVersion" == "$hisVersion" ]]; then
    printf "\n[Yui] → A versão da GitHub já é a mesma versão que você está usando, deseja continuar?\n\n"
else
    printf "\n[Yui] → Existe uma atualização disponível para você, deseja continuar?\n\n"
fi
...
select choice in "Sim" "Não"; do
    case $choice in
                ...
                case $backupOption in
                    "Sim (y)")
                        ...
                        if [[ -f "main.zip" ]]; then
                            ...
                            # Avisa que vai atualizar
                            printf "\n[Yui] → Movendo códigos atualizados...\n"

                            # Move os arquivos e pastas, excluindo Toolbox.sh e Reinstall.sh
                            find Yui-main/lib/Scripts -mindepth 1 -maxdepth 1 ! -name "Reinstall.sh" ! -name "Toolbox.sh" -exec mkdir -p "./lib/Scripts/" \; -exec mv {} ./lib/Scripts/ \;

                            # Move o resto da pasta lib, exceto scripts
                            find Yui-main/lib -mindepth 1 -maxdepth 1 ! -name 'Scripts' -exec mv {} ./lib/ \;

                            # Move tudo de Yui-main para o diretório atual, excluindo a pasta lib
                            find Yui-main -mindepth 1 -maxdepth 1 ! -name "lib" -exec mv {} . \;

                            # Move o cache dos zip para o backup
                            mv main.zip "${folderName}"
                            mv Yui-main "${folderName}"
                            ...
                            # Avisa que atualizou
                            printf "\n[Yui] → Fui atualizada com sucesso, prosseguindo para etapas de configuração JSON...\n"

                            # Pergunta ao usuário se deseja fazer a auto-configuração dos arquivos da pasta 'Settings'
                            printf "\n[Yui] → Deseja fazer a auto-configuração dos arquivos da pasta 'Configurations' usando os antigos arquivos?\n\nValores que não existem na versão baixada da Yui serão ignorados.\n\nEsta opção vai configurar as API's e demais configurações automaticamente para você, desde que você tenha configurado anteriormente, antes da atualização.\n\n"
                            ...
                            select autoConfigOption in "Sim (y)" "Não (n)"; do
                                case $autoConfigOption in
                                    ...
                                    "Sim (y)")
                                        printf "\n[Yui] → Essa função será executada pelo NodeJS, avisarei quando terminar!\n\n"
                                        ...
                                        # Avisa terminar
                                        printf "\n[Yui] → Terminado, suas configurações foram importadas.\n"
                                        break
                                    ;;

                                    # Não configura automatico
                                    "Não (n)")
                                        printf "\n[Yui] → Okay, irei pular a configuração dos arquivos.\n"
                                        break
                                    ;;
                                esac
                            done
                            ...
                            printf "\n[Yui] → Deseja atualizar os módulos e instalar (1 ou 2)?\n\n"
                            select startOption in "Sim (y)" "Não (n)"; do
                                case $startOption in
                                    ...
                                    # Ignora
                                    "Não (n)")
                                        printf "\n[Yui] → Entendido, vamos para a próxima pergunta!\n"
                                        break
                                    ;;
                                esac
                            done
                            ...
                            printf "\n[Yui] → Algum erro aconteceu durante o download da atualização.\n"
                            exit 1
                        fi
                    ;;
                    ...
                    # Cancela
                    "Não (n)")
                        printf "[Yui] → Ok, não irei ter minha querida atualização...\n"
                        exit 0
                    ;;
                    ...
                    "Download ZIP (z)")
                        ...
                        # Verifica se o download foi concluído com sucesso
                        if [[ -f "main.zip" ]]; then
                            printf "[Yui] → Minha atualização foi baixada com sucesso! Você pode abrir clicando no arquivo 'main.zip'.\n"
                            exit 0
                        else
                            # Se falhou avisa e sai com erro 1
                            printf "[Yui] → Houve algum erro durante o download da atualização.\n"
                            exit 1
                        fi
                    ;;
                esac
        ...
        # Sai do atualizador
        "Não")
            printf "[Yui] → Você decidiu cancelar a atualização, se quiser algo mais, basta rodar novamente."
            exit 0
        ;;
    esac
done
