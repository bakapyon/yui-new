...
if [ -d "node_modules" ]; then
    printf "[Yui] → Você já instalou, deseja atualizar os módulos? (1 ou 2)?\n\n"
    ...
    printf "[Yui] → Instalando, abaixo está a output do NPM:\n"
    ...
    if npm i; then
        ...
        select dsp in "Sim (y)" "Não (n)"; do
            case $dsp in
                ...
                # Sai sem erros
                "Não (n)")
                    printf "[Yui | DONE] → Ok, obrigado por utilizar este programa, módulos instalados! <3\n"
                    exit 0
                ;;
