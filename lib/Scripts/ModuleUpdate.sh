...
if [ -d "node_modules" ]; then
    printf "\n[Yui] → Função iniciada, output do NPM -> \n"

    # Executa a atualização e verifica se ocorreram erros
    if npm update; then
        # Desatualiza o Sharp se quiser
        printf "[Yui] → Deseja fazer downgrade do módulo sharp (Windows é obrigatório!)?\n\n"
        select dsp in "Sim (y)" "Não (n)"; do
            case $dsp in
                # Executa o script de downgrade
                "Sim (y)")
                    # Avisa para ignorar os WARNs
                    printf "[Yui] → Avisos de 'WARN' NÃO SÃO ERROS neste caso, não se preocupe com eles, se tudo correr bem avisarei.\n\n"
                    ...
                    if npm i sharp@0.30.7; then
                        ...
                        printf "[Yui | DONE] → Instalação concluída com sucesso! <3\n"
                        exit 0
                    else
                        printf "[Yui | ERRO] → Ocorreram erros durante a instalação. Verifique o console e procure o suporte.\n"
                        exit 1
                    fi
                ;;
                ...
                # Sai sem erros
                "Não (n)")
                    printf "[Yui | DONE] → Ok, obrigado por utilizar este programa, módulos atualizados! <3\n"
                    exit 0
                ;;
        ...
        printf "[Yui | ERRO] → Ocorreram erros durante a atualização. Verifique o console e procure o suporte.\n"
        exit 1
    fi
else
    # Se os módulos não estão instalados, oferece a opção de instalar
    printf "[Yui] → Você não fez a instalação, deseja instalar os módulos (1 ou 2)?\n\n"
    select opt in "Sim (y)" "Não (n)"; do
        case $opt in
            ...
            "Não (n)")
                printf "[Yui | DONE] → Ok, obrigado por utilizar este programa! <3\n"
                exit 0
            ;;
