...
async function genSession(launchOpt, i, launchInstancer) {
    ...
    kill = await makeWASocket(startOptions);
...
/* Inicia uma ou mais sessões da Yui | Try-Catch para casos de erro */
    ...
    try {
        ...
        for (let i = 0; i < launchInstance.sessionsLength; i += 1) {
            ...
            if (config.pairingCode.value && !client.kill.authState.creds.registered) {
                ...
                const phoneNumber = await question('Digite o número da sua Yui (e.g: 55123409876): ');
